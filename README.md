...
- Debug information is retained through each builder's pre-export checks, then
  stripped from the shipped copies. Fil-C panic metadata and identifying
  symbols remain.
...
Debug information is retained through the build because it permits structural
checks before stripping. Before export, every native executable is checked for:
